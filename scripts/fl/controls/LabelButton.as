package fl.controls
{
   ...
   public class LabelButton extends BaseButton implements IFocusManagerComponent
   {
      ...
      {
         return !!this._toggle ? Boolean(_selected) : false;
      }
      ...
      override protected function drawLayout() : void
      {
         ...
         else if(_loc2_ == ButtonLabelPlacement.BOTTOM || _loc2_ == ButtonLabelPlacement.TOP)
         {
            ...
            if(this.icon != null)
            {
               this.icon.y = Math.round(_loc2_ == ButtonLabelPlacement.BOTTOM ? Number(this.textField.y - _loc6_) : Number(this.textField.y + this.textField.height + _loc1_));
            }
         }
         else
         {
            _loc7_ = Math.max(0,Math.min(_loc3_,width - _loc5_ - 2 * _loc1_));
            this.textField.x = Math.round((width - (this.textField.width = Number(_loc7_)) - _loc5_) / 2 + (_loc2_ != ButtonLabelPlacement.LEFT ? _loc5_ : 0));
            this.textField.y = Math.round((height - this.textField.height) / 2);
            if(this.icon != null)
            {
               this.icon.x = Math.round(_loc2_ != ButtonLabelPlacement.LEFT ? Number(this.textField.x - _loc5_) : Number(this.textField.x + _loc3_ + _loc1_));
            }
         }
         super.drawLayout();
      }
